...
`@qubiq/core/serialization` keeps the tricky byte layouts close to the original C++ codebase. Downstream SDKs can reuse these helpers for signing flows, transaction builders, or gRPC bridges.
