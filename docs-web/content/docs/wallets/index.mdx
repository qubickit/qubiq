...
`@qubickit/core/wallet` exposes everything you need to build wallet-aware libraries, CLIs, or desktop apps. This page summarizes the key APIs and how they compose.

## Deterministic derivation

```ts
import { deriveWalletFromSeed } from "@qubickit/core";
...
```ts
import { deriveWalletFromPath } from "@qubickit/core/wallet";
...
```ts
import { identityToPublicKey, publicKeyToIdentity, normalizeIdentity } from "@qubickit/core/wallet";
...
```ts
import { WalletWatcher } from "@qubickit/core";
...
- Desktop wallet prototype (`@qubickit/wallet`) will consume these primitives.
