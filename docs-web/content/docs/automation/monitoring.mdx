...
description: Collect ticks, balances, HTTP timings, and export Prometheus metrics from @qubickit/core.
---

This module (`@qubickit/core/monitoring`) keeps production services observable. Use it alongside automation pipelines, connectors, or your own workers.

## Tick & Balance Monitors

```ts
import { TickMonitor, BalanceMonitor } from "@qubickit/core/monitoring";
import { LiveServiceClient } from "@qubickit/core/clients";
...
```ts
import { TelemetryMetricsRegistry } from "@qubickit/core/monitoring";
...
```ts
import { PrometheusMetricsServer } from "@qubickit/core/monitoring";
...
- `qubickit_tick_info{}` — latest tick, epoch, duration.
- `qubickit_balance{identity}` — tracked identities.
- `qubickit_http_request_duration_seconds_bucket|sum|count`.
- `qubickit_http_request_errors_total{name}`.

</details>

## Request instrumentation

Wrap any async call (HTTP, gRPC, DB) with `instrumentRequest`:

```ts
import { instrumentRequest } from "@qubickit/core/monitoring";
