...
Import everything from `@qubickit/core/types` (or the narrower `@qubickit/core/types/rpc`) when you need fully typed data structures that also validate at runtime via Zod.

## Core schemas

```ts
import { TransactionSchema, ProposalDataSchema, MessageType } from "@qubickit/core/types";
...
```ts
import { IdentityStringSchema, HashStringSchema } from "@qubickit/core/types/rpc";
