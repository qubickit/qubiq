...
```ts
import { BaseConnector } from "@qubickit/core";
...
```ts
import { HttpConnector } from "@qubickit/core";
...
```ts
import { createHttpConnector } from "@qubickit/core";
...
`QubicNodeClient` lives under `@qubickit/core/node` and acts like an SDK-ready façade:
