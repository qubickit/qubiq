...
`@nexakit/core/config` centralizes knobs for live, query, and archive services so you can boot the library in different environments (prod, staging, air-gapped).

## ClientConfig basics

```ts
import type { ClientConfig } from "@nexakit/core";
import { createClients } from "@nexakit/core";
