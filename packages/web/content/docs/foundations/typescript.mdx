...
description: How to run @nexakit/core locally, explore examples, and extend the toolkit.
---

This project is designed to feel great in everyday workflows—Bun for scripts/tests, Fumadocs for docs, and a set of runnable examples that mirror the guides on [core.nexakit.dev](https://core.nexakit.dev).
...
Use the same commands inside `packages/web/` to work on the documentation site:
...
Need a quick gRPC playground? See `tests/interop/grpc-live-service.test.ts` for a fully in-process mock server you can adapt.

## Recommended workflow

1. Start the docs site (`bun dev` in `packages/web/`).
...
That’s it—the rest of the docs dive into modules like wallets, automation, interop, and monitoring, all available at [core.nexakit.dev](https://core.nexakit.dev).
