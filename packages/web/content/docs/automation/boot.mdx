...
`@nexakit/core/boot` ships a small orchestration layer that mirrors the native node’s boot flags. It’s ideal for validators, custodial services, or automation daemons that need to resume safely after downtime.

## BootManager

```ts
import { BootManager, BootMode } from "@nexakit/core/boot";
