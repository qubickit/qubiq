...
description: Collect ticks, balances, HTTP timings, and export Prometheus metrics from @nexakit/core.
---

This module (`@nexakit/core/monitoring`) keeps production services observable. Use it alongside automation pipelines, connectors, or your own workers.

## Tick & Balance Monitors

```ts
import { TickMonitor, BalanceMonitor } from "@nexakit/core/monitoring";
import { LiveServiceClient } from "@nexakit/core/clients";
...
```ts
import { TelemetryMetricsRegistry } from "@nexakit/core/monitoring";
...
```ts
import { PrometheusMetricsServer } from "@nexakit/core/monitoring";
...
- `nexakit_tick_info{}` — latest tick, epoch, duration.
- `nexakit_balance{identity}` — tracked identities.
- `nexakit_http_request_duration_seconds_bucket|sum|count`.
- `nexakit_http_request_errors_total{name}`.
...
```ts
import { instrumentRequest } from "@nexakit/core/monitoring";
