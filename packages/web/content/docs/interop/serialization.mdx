...
`@nexakit/core/serialization` keeps the tricky byte layouts close to the original C++ codebase. Downstream SDKs can reuse these helpers for signing flows, transaction builders, or gRPC bridges.

## Transaction encoding

```ts
import { encodeTransaction, decodeTransaction } from "@nexakit/core/serialization";
...
} from "@nexakit/core/serialization";
...
Re-exported from `@nexakit/core/utils/bytes`:
