...
```ts
import { BaseConnector } from "@nexakit/core";
...
```ts
import { HttpConnector } from "@nexakit/core";
...
```ts
import { createHttpConnector } from "@nexakit/core";
...
`QubicNodeClient` lives under `@nexakit/core/node` and acts like an SDK-ready façade:

```ts
import { QubicNodeClient } from "@nexakit/core";
